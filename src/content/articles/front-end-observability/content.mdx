import { CodeSample, MdxCode, MdxPre } from "@/components";
...
2. `useShareholderStakeEditor/changeStakeSumError` - stakes sum was too high, so we will show an error2

<CodeSample language="javascript" lineNumbers linesHighlighted={[[8,23], [27,33], [42,48]]}>{`
function ShareholderStakeEditor({
  companyId,
  shareholder,
  onClickClose,
